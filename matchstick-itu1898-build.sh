#!/bin/bash
TARGET_BRANCH="${TARGET_BRANCH-matchstick-itu1898}"
...
cd $SRC_DIR
BRANCH=matchstick REPO_INIT_FLAGS='--repo-url=appler:tools/repo.git' ./config.sh -d itu1898 >> repo.log 2>&1

#source load-config.sh
BUILD_PROP=$SRC_DIR/device/rockchip/rk3188/rk3188.mk
$(modify_prop "ro.product.platform" "MS-ITU1898" $BUILD_PROP)
...
sudo cp -r $SRC_DIR/manifest.xml $SRC_DIR/$TARGET_BRANCH-$DATE_TIME.zip $SRC_DIR/out/target/product/rk3188/obj/PACKAGING/target_files_intermediates/*.zip $SRC_DIR/out/target/product/rk3188/*.zip /mnt/public/$DEST_DIR/
