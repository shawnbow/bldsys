#!/bin/bash
TARGET_BRANCH="${TARGET_BRANCH-vision}"
SRC_DIR=~/data/build/$TARGET_BRANCH
...
cd $SRC_DIR
REPO_INIT_FLAGS='--repo-url=appler:tools/repo.git' ./config.sh -d rk30sdk-kk >> repo.log 2>&1
...
sudo cp -r $SRC_DIR/manifest.xml $SRC_DIR/$TARGET_BRANCH-$DATE_TIME.zip /mnt/public/$DEST_DIR/
